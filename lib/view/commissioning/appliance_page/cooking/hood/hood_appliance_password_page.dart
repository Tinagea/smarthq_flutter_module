...
class HoodEnterPasswordStep extends StatefulWidget {
  HoodEnterPasswordStep({Key? key}) : super(key: key);

  _HoodEnterPasswordStep createState() => _HoodEnterPasswordStep();
}

class _HoodEnterPasswordStep extends State<HoodEnterPasswordStep> {
      ...
      child: WillPopScope(
        ...
        child: Scaffold(
          ...
          appBar: CommonAppBar(
              ...
              leftBtnFunction: () {
                if (globals.routeNameToBack == Routes.HOOD_DESCRIPTION2_MODEL2) {
                  CommissioningUtil.navigateBackAndStopBlescan(context: context);
                  Navigator.of(context).popUntil((route) => route.settings.name == Routes.HOOD_DESCRIPTION2_MODEL2);
                }else{
                        ...
                        child: ListView(
                          ...
                          children: <Widget>[
                            Component.componentMainImage(context,
                                ImagePath.HOOD_PASSWORD_IMAGE),
                            BaseComponent.heightSpace(16.h),
                            Visibility(
                              child: Component.componentDescriptionText(
                                  text:
                                  LocaleUtil.getString(
                                      context, LocaleUtil.APPLIANCE_NETWORK_INFO)!,
                          ...
                          onTapButton: () {
                            ...
                            if (_isAutoJoinSupport) {
                              loadingDialog.show(context,
                                  LocaleUtil.getString(
                                      context, LocaleUtil.CONNECTING_TO_YOUR_APPLIANCE));
                              ...
                              Navigator.of(context, rootNavigator: true).pushNamed(Routes.COMMON_MAIN_NAVIGATOR).then(
                                      (_) => BlocProvider.of<BleCommissioningCubit>(context).restartContinuousScanForAppliance());
                            }
                          })
                    ],
                  );
