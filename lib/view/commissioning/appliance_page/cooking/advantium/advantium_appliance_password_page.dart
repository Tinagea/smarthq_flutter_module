...
class PageAdvantiumPassword extends StatefulWidget {
  final bool isFirstTimeShown;
  PageAdvantiumPassword({
    Key? key, required this.isFirstTimeShown,
  }) : super(key: key);

  @override
  _PageAdvantiumPassword createState() => _PageAdvantiumPassword(isFirstTimeShown: this.isFirstTimeShown);
}

class _PageAdvantiumPassword extends State<PageAdvantiumPassword> with WidgetsBindingObserver {
  bool _isFirstTimeShown = false;

  _PageAdvantiumPassword({bool isFirstTimeShown = false}) {
    ...
    return FocusDetector(
      ...
      child: WillPopScope(
        ...
        child: Scaffold(
          ...
          appBar: CommonAppBar(title: LocaleUtil.getString(context, LocaleUtil.ADD_APPLIANCE),
            leftBtnFunction: () {
              if (!_isFirstTimeShown) {
                CommissioningUtil.navigateBackAndStopBlescan(context: context);
                Navigator.of(context).popUntil((route) => route.settings.name == Routes.ADVANTIUM_DESCRIPTION);
              }else{
                      ...
                      children: [
                        Expanded(
                          child: ListView(
                            ...
                            shrinkWrap: true,
                            children: <Widget>[
                              Component.componentMainImage(context, ImagePath.ADVANTIUM_PASSWORD_INFO),
                              ...
                              CustomRichText.customSpanListTextBox(textSpanList: <TextSpan>[
                                TextSpan(text: LocaleUtil.getString(context, LocaleUtil.ADVANTIUM_PASSWORD_TEXT_1), style: textStyle_size_18_color_white()),
                                TextSpan(text: LocaleUtil.getString(context, LocaleUtil.ADVANTIUM_PASSWORD_TEXT_2), style: textStyle_size_18_color_yellow()),
                                TextSpan(text: LocaleUtil.getString(context, LocaleUtil.ADVANTIUM_PASSWORD_TEXT_3), style: textStyle_size_18_color_white())
                              ]),
                              BaseComponent.heightSpace(16.h),
                              ...
                              return Component.componentBottomButton(
                                  ...
                                  onTapButton: () {
                                    if (_isFirstTimeShown &&
                                        (_passwordTextFieldStatus == TextFieldStatus.empty) &&
                                        (_geModuleTextFieldStatus == TextFieldStatus.empty)) {
                                      Navigator.of(context).pushNamed(Routes.ADVANTIUM_DESCRIPTION);
